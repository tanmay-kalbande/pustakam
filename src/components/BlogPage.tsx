...
const BarChartVisual: React.FC<{ data: { label: string; value: number; color: string }[]; title: string }> = ({ data, title }) => {
    ...
    return (
        <div className="my-8 p-6 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-subtle)]">
            <h4 className="text-xs font-bold text-[var(--text-secondary)] mb-6 uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="space-y-4">
                {data.map((item, i) => (
                    <div key={i} className="flex items-center gap-4">
                        <span className="text-[10px] text-[var(--text-muted)] w-24 shrink-0 text-right font-medium">{item.label}</span>
                        <div className="flex-1 h-6 bg-[var(--bg-base)] rounded-md overflow-hidden relative border border-[var(--border-subtle)]">
                            <div
                                className="h-full rounded-sm transition-all duration-1000 ease-out flex items-center justify-end pr-3"
                                style={{ width: `${(item.value / max) * 100}%`, background: item.color.includes('gradient') ? item.color : 'var(--brand)' }}
                            >
                                <span className="text-[9px] font-bold text-white drop-shadow-sm">{item.value}%</span>
                            </div>
                        </div>
                    </div>
                ))}
...
const StatGrid: React.FC<{ stats: { label: string; value: string; icon: React.ElementType; color: string }[] }> = ({ stats }) => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-8">
        {stats.map((stat, i) => (
            <div key={i} className="p-5 rounded-lg bg-[var(--bg-surface)] border border-[var(--border-subtle)] text-center group hover:border-[var(--border-default)] transition-all duration-300">
                <stat.icon size={20} className={`mx-auto mb-3 text-[var(--brand)] opacity-80 group-hover:scale-110 transition-transform`} />
                <div className="text-2xl font-bold text-[var(--text-primary)] mb-1">{stat.value}</div>
                <div className="text-[10px] text-[var(--text-muted)] uppercase tracking-widest font-bold">{stat.label}</div>
            </div>
        ))}
    </div>
);
...
const BLOG_POSTS: BlogPost[] = [
    {
        ...
        content: (
            <>
                ...
                <StatGrid stats={[
                    ...
                    { label: 'Writing Modes', value: '2', icon: Flame, color: 'text-blue-400' },
        ...
        ctaText: "Start learning better. Build your first book.",
    },
    {
        ...
        content: (
            <>
                ...
                <p>Personalized content is useless if the delivery is boring. Standard AI writing is safe and sanitized. That's why I honestly prefer **Street Mode** for 90% of my own research. It cuts the fluff and dares you to execute. It’s what actual learning feels like outside the lecture hall.</p>

                <p>Standard academic writing is dead. If you're stuck in tutorial hell, let Street Mode roast you into action.</p>
            </>
        ),
        ctaText: "Get Roasted. Try Street Mode.",
    },
    {
        ...
        content: (
            <>
                ...
                <h3>The Single-Source Fuel: Zhipu GLM</h3>

                <p>Initially, I experimented with a multi-model approach. But frankly, managing token rates and context windows across five different APIs was a headache. To build a product that felt *reliable* rather than just *versatile*, I standardized on the Zhipu GLM ecosystem via a secure proxy pipeline.</p>

                <p>We use **GLM-5** for the core chapter synthesis because it writes contextually rich, long-form content. We offload structural assembly to **GLM-5 Turbo** and use **FlashX** for rapid enhancement tasks. It’s cleaner, faster, and lets me tune the performance perfectly for book generation.</p>
                ...
                <InfoCallout
                    ...
                    text="Traffic is routed securely through a verified node before reaching Zhipu endpoints. Your authentication tokens are handled securely, and no middleman reads your books. It's built for stability, not data scraping."
                />
        ...
        ctaText: "See the pipeline in action. Start generating.",
    },
    {
        ...
        ctaText: "Stop overthinking. Build your first book now.",
    },
    {
        ...
        content: (
            <>
                <p>Here's a stake in the ground: traditional online courses are dead. The model of "one instructor records one series of videos and thousands watch it" is lazy and inefficient.</p>

                <p>Why? Because a video cannot pause to address your specific confusion. It doesn't matter how great the teacher is; a one-size-fits-all lecture is always a compromise. Pustakam solves this by building content one-to-one.</p>
                ...
                <p>Within the next 2 years, we'll see a fundamental shift:</p>
        ...
        ctaText: "Join the future of learning. Generate your book.",
    },
];
                ...
                <article className="max-w-3xl mx-auto px-6 py-12 relative z-10">
                    ...
                    <div className="prose-pustakam">
                        ...
                        {selectedPost.ctaText && (
                            <p className="font-bold text-cyan-400 mt-8">
                                <button onClick={() => { setSelectedPostId(null); onClose(); }} className="hover:underline flex items-center gap-1 group">
                                    → {selectedPost.ctaText}
        ...
        <div className="fixed inset-0 z-[100] bg-[#050505] overflow-auto">
            ...
            <header className="sticky top-0 z-10 bg-[#050505]/90 backdrop-blur-xl border-b border-[var(--border-subtle)] px-6 py-4">
                <div className="max-w-5xl mx-auto flex items-center justify-between">
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface)] transition-colors"
                    >
                        <X size={18} />
                    </button>
                </div>
            ...
            <main className="max-w-5xl mx-auto px-6 py-12 relative z-10">
                ...
                {(() => {
                    ...
                    return (
                        ...
                        >
                            <span role="presentation" className="block w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {BLOG_POSTS.slice(1).map((post) => (
                        ...
                        >
                            <span role="presentation" className="block h-full">

    ...
    constructor(
        ...
        super(
            ...
            provider,
            {
                ...
                retryAfter,
                details: { model },
                userMessage: `Rate limit hit for ${provider}. Relax for a minute or switch models.`,
            }
        );
...
export class NetworkError extends PustakamError {
    constructor(originalMessage?: string) {
        super(
            ...
            {
                recoverable: true,
                userMessage: 'Connection lost. Check your Wi-Fi.',
            }
        );
...
export class AuthenticationError extends APIError {
    constructor(provider: string) {
        super(
            ...
            {
                statusCode: 401,
                userMessage: `Key expired for ${provider}. Double check settings.`,
            }
        );
...
export class GenerationError extends PustakamError {
    ...
    constructor(
        ...
        super(message, `GENERATION_${phase.toUpperCase()}_FAILED`, {
            details: options?.details,
            recoverable: true,
            userMessage: options?.userMessage ?? `Failed to generate ${phase}. Hit retry.`,
        });
...
export function parseAPIError(error: unknown, provider: string): PustakamError {
    ...
    return new APIError(
        ...
        provider,
        {
            userMessage: `Something broke communicating with ${provider}. Try again in a sec.`,
        }
    );
}

    ...
    start(controller) {
      ...
      void (async () => {
        try {
          ...
          if (!providerRes.ok) {
            const providerError = await providerRes.text().catch(() => providerRes.statusText);
            await failRequest(`AI provider error (${providerRes.status}): ${providerError}`);
            return;
          }
          ...
          // Pricing values are per million tokens in USD
          const costUsd =
